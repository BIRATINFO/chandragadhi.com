...
export default async function Page() {
  ...
  const roadblockRes = await fetch(`${backend_uri}/api/roadblocks/network/chandragadhi.com`, options)
  ...
  const [heroData, categoryBlocks] = await Promise.all([
    fetch(`${backend_uri}/api/posts/featured-news/chandragadhi.com`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => []),

    fetch(`${backend_uri}/api/posts/homepage/chandragadhi.com`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => [])
  ])
